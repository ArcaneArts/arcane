...
class _TypePicker extends StatelessWidget {
  ...
  const _TypePicker({required this.type, required this.onChanged});

  @override
  Widget build(BuildContext context) => PopupMenu(icon: (Icons.package), items: [...NavigationType.values.map((e) => MenuCheckbox(onChanged: (context, v) => onChanged(e), value: e == type, child: Text(e.name)))]);
}
...
class _ExampleNavTabsState extends State<ExampleNavTabs> {
  ...
  @override
  Widget build(BuildContext context) => NavigationScreen(type: type, onIndexChanged: (index) => setState(() => this.index = index), index: index, tabs: [
        NavTab(
            icon: Icons.house,
            ...
            builder: (context, footer) => FillScreen(
                ...
                header: Bar(
                  titleText: "Home",
                  trailing: [_TypePicker(type: type, onChanged: (t) => setState(() => type = t))],
                ),
                child: CenterBody(
                  icon: Icons.alien,
                  message: "The Message",
