...
class DataTable extends m.DataTable {
  ...
  void _handleSelectAll(bool? checked, bool someChecked) {
    ...
    if (onSelectAll != null) {
      ...
      for (final m.DataRow row in rows) {
  ...
  /// Note: to change background color of fixed data rows use [DataTable.headingRowColor]
  ...
  Widget build(BuildContext context) {
    ...
    final anyRowSelectable =
        rows.any((m.DataRow row) => row.onSelectChanged != null);
    final displayCheckboxColumn = showCheckboxColumn && anyRowSelectable;
    final rowsWithCheckbox = displayCheckboxColumn
        ? rows.where((m.DataRow row) => row.onSelectChanged != null)
        : <DataRow>[];
    final rowsChecked = rowsWithCheckbox.where((m.DataRow row) => row.selected);
    final allChecked =
      ...
      return SyncedScrollControllers(
          ...
          builder: (context, sc11, sc12, sc21, sc22) {
            ...
            for (int dataColumnIndex = 0;
                ...
                dataColumnIndex++) {
              final m.DataColumn column = columns[dataColumnIndex];
              ...
              for (final m.DataRow row in rows) {
                ...
                var c = _buildDataCell(
                    ...
                    padding: padding,
                    specificRowHeight:
                        row is DataRow ? row.specificRowHeight : null,
                    label: cell.child,
                    numeric: column.numeric,
                    ...
                    onRowTap: row is DataRow ? row.onTap : null,
                    onRowDoubleTap: row is DataRow ? row.onDoubleTap : null,
                    onRowLongPress: row.onLongPress,
                    onRowSecondaryTap:
                        row is DataRow ? row.onSecondaryTap : null,
                    onRowSecondaryTapDown:
                        row is DataRow ? row.onSecondaryTapDown : null,
                    ...
                    Scrollbar(
                        ...
                        child: SingleChildScrollView(
                            ...
                            child: SingleChildScrollView(
                                ...
                                child: a.PaddingTop(
                                    padding: effectiveHeadingRowHeight,
                        ...
                        child: Scrollbar(
                            ...
                            child: SingleChildScrollView(
                                ...
                                child: (fixedRowsTabel != null)
                                    ? a.SurfaceCard(
                                        borderRadius: a.BorderRadius.zero,
    ...
    if (displayCheckboxColumn) {
      ...
      for (final m.DataRow row in rows) {
        var x = _buildCheckbox(
            ...
            onRowTap: () {
              if (row is DataRow && row.onTap != null) {
                row.onTap?.call();
              } else {
            ...
            tristate: false,
            rowHeight: rows[rowIndex] is DataRow
                ? (rows[rowIndex] as DataRow).specificRowHeight
                : null);
  ...
  List<double> _calculateDataColumnSizes(BoxConstraints constraints,
    ...
    var totalFixedWidth = columns.fold<double>(
        ...
        (previousValue, element) =>
            previousValue +
            (element is DataColumn && element.fixedWidth != null
                ? element.fixedWidth!
                : 0.0));
    ...
    final widths = List<double>.generate(columns.length, (i) {
      ...
      if (!(column is DataColumn && column.fixedWidth != null)) {
    ...
    for (var i = 0; i < widths.length; i++) {
      ...
      if (!(columns[i] is DataColumn &&
          (columns[i] as DataColumn).fixedWidth != null)) {
        widths[i] *= ratio;
      }
    }
      ...
      (int index) {
        ...
        if (headingRow != null && index == 0) {
          ...
          Decoration? rowDecoration =
              rows[rowStartIndex + actualIndex] is DataRow
                  ? (rows[rowStartIndex + actualIndex] as DataRow).decoration
                  : null;
