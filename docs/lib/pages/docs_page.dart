  ...
  Widget buildBadge(BuildContext context) {
    ...
    switch (this) {
      ...
      case ShadcnFeatureTag.updated:
        ...
        badgeText = 'Updated';
        break;
      case ShadcnFeatureTag.arcane:
        copy = theme.copyWith(
          colorScheme: theme.colorScheme.copyWith(
            primary: const Color(0xFF5a2af7),
          ),
        );
        badgeText = 'Arcane';
        break;
...
class DocsPageState extends State<DocsPage> {
  static final List<ShadcnDocsSection> sections = [
    ...
    ShadcnDocsSection("Arcane", [
      ShadcnDocsPage('Screens', 'arcane_screens'),
      ShadcnDocsPage('Dialogs', 'arcane_dialogs'),
      ShadcnDocsPage('Toast', 'arcane_toast'),
    ]),
  ...
  void showSearchBar() {
    DialogCommand(
      hint: "Search Docs",
      options: {
        ...sections.expand((e) => e.pages).map((e) => e.title),
      },
      onConfirm: (t) {
        ShadcnDocsPage? f = sections
            .expand((e) => e.pages)
            .where((e) => e.title == t)
            .firstOrNull;

        if (f != null) {
          context.goNamed(f.name);
        } else {
          const TextToast('Page not found').open(context);
        }
      },
    ).open(context);
                          ...
                          alternateChild: AppBar(
                            ...
                            leading: [
                              GhostButton(
                                ...
                                child: const Icon(Icons.menu_ionic),
                              ),
                            ],
                            trailing: [
                              GhostButton(
                            ...
                            child: Center(
                              child: SizedBox(
                                ...
                                child: OutlineButton(
                                  ...
                                  trailing: const Icon(Icons.search_ionic)
                                      .iconSmall()
                                      .iconMutedForeground(),
  ...
  AppBar _buildAppBar(EdgeInsets padding, ThemeData theme) {
    return AppBar(
      ...
      padding: padding,
      title: Basic(
        leading: ArcaneArtsLogo(
          color: const Color(0xFF3a32a8),
          size: 32 * theme.scaling,
        ),
        subtitle: Text("Tome of Structure"),
        title: const Text(
          'Arcane',
        ),
      ),
